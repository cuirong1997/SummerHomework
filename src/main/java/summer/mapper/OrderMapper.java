...
public interface OrderMapper {
	...
	Good selectGood(@Param("good_id")Integer good_id);
	
	@Select("select * from tb_order where order_uid=#{user_id} and order_gid=#{good_id}")
	Order findGoodById(@Param("user_id")Integer user_id,@Param("good_id")Integer good_id);
	
	@Select("update tb_order set order_number=#{good_number} where order_uid=#{user_id} and order_gid=#{good_id}")
	Order updatenumber(@Param("user_id")Integer user_id,@Param("good_id")Integer good_id,@Param("good_number")Integer good_number);
	
	@Insert("insert into tb_order(order_uid,order_gid,order_name,order_price,order_number,order_image,order_address,order_status) "
			+ "				values(#{user_id},#{good_id},#{order_name},#{order_price},#{good_number},#{order_image},#{order_address},#{order_status})")
	void insertorder(@Param("user_id")Integer user_id,@Param("good_id")Integer good_id,@Param("order_name")String order_name,
			@Param("order_price")int order_price,@Param("good_number")int good_number,
			@Param("order_image")String order_image,@Param("order_address")String order_address,@Param("order_status")String order_status);
	
	@Select("select * from tb_order where order_uid=#{user_id}")
	List<Order> readorder(@Param("user_id")Integer user_id);
	...
	Good updatestandse(@Param("good_store")int good_store,@Param("good_sell")int good_sell,@Param("good_id")Integer good_id);
